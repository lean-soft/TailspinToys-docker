...
//	Configuration
//
var fileLoadingImage = "fw/lightbox/loading.gif";				// Loading image		
var fileBottomNavCloseImage = "fw/lightbox/closelabel.gif";		// Close image
var fallbackOverlayImage = "fw/lightbox/overlay.png";			// Fallback overlay used with browsers that have opacity problems
...
Lightbox.prototype = {
	...
	initialize: function() {	
		...
		objLightboxFlv = document.createElement("div");
		objLightboxFlv.setAttribute('id','lightboxflv');
		objLightboxFlv.style.position = 'relative';
		objLightboxFlv.style.zIndex = 101; // Safari
		objLightboxFlv.style.display = 'none';
		objContent.appendChild(objLightboxFlv);
	...
	start: function(elementLink) {	
		...
		if((elementLink.getAttribute('rel') == 'lightbox')){
			...
			if(elementLink.getAttribute('href').endsWith('swf') || elementLink.getAttribute('href').endsWith('flv')) { // SWF/FLV
		...
		// Image or SWF is not part of a set...
			...
			for (var i=0; i<anchors.length; i++){
				...
				if (anchor.getAttribute('href') && (anchor.getAttribute('rel') == elementLink.getAttribute('rel'))){
					if(anchor.getAttribute('href').endsWith('swf') || anchor.getAttribute('href').endsWith('flv')) { // SWF/FLV
	...
	changeElement: function(elementNum) {	
		...
		if (elementArray[activeElement][0].endsWith('swf') || elementArray[activeElement][0].endsWith('flv')){ // SWF/FLV
	...
	showElement: function(){
		...
		} else if(elementArray[activeElement][0].endsWith('flv')) { // FLV
			// Since the activeElement is a FLV, hide the image element.
			$('lightboximage').style.display = 'none';
			// Setup the proper object and embed tags in the FLV div.  The following is a basic set of object and embed tags, edit as you see fit.
			var obj2 = $('lightboxflv');
			obj2.innerHTML = "<object classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0\" width=\""+elementArray[activeElement][2]+"\" height=\""+elementArray[activeElement][3]+"\">" +
				"<param name=\"movie\" value=\"flvPlayer.swf\"/>" +
				"<param name=\"quality\" value=\"high\" />" +
				"<param name=\"allowscriptaccess\" value=\"always\" />" +
				"<param name=\"wmode\" value=\"transparent\" />" +
				"<param name=\"flashvars\" value=\"autostart=true&amp;file="+elementArray[activeElement][0]+"\&amp;width="+elementArray[activeElement][2]+"&amp;height="+elementArray[activeElement][3]+"\" />" +
				"<embed wmode=\"transparent\" allowscriptaccess=\"always\" src=\"flvPlayer.swf\" quality=\"high\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" type=\"application/x-shockwave-flash\" width=\""+elementArray[activeElement][2]+"\" height=\""+elementArray[activeElement][3]+"\" FlashVars=\"autostart=true&amp;file="+elementArray[activeElement][0]+"\&amp;width="+elementArray[activeElement][2]+"&amp;height="+elementArray[activeElement][3]+"\">" +
				"</embed>" +
				"</object>";
			new Effect.Appear('lightboxflv', {  duration: resizeDuration, queue: 'end', from: 0.0, to: 100, afterFinish: function(){ myLightbox.updateDetails(); } });
		} else { // Image
	...
	//
	//	updateDetails()
	//	Display caption, image/SWF/FLV number, and bottom nav.
	//
	updateDetails: function() {
		...
		// if image/SWF/FLV is part of set display 'Image/Movie x of x' 
